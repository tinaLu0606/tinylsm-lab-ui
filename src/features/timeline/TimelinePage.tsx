  ...
  return (
    <div className="page-stack">
      ...
      <section className="metric-grid">
        ...
        <article className="metric-card"><span>{state.source === 'live' ? 'Sampled throughput' : 'Mock throughput'}</span><strong>{latestPoint?.operationsPerSecond ?? 0}</strong><small>{state.source === 'live' ? 'Goal 3 sampling pending' : 'operations / second'}</small></article>
        <article className="metric-card"><span>P95 engine latency</span><strong>{formatMicros(latestPoint?.p95Micros ?? 0)}</strong><small>{state.source === 'live' ? 'Goal 3 aggregation pending' : 'deterministic mock value'}</small></article>
