  ...
  return (
    <div className="page-stack">
      <section className="hero-panel">
        ...
        <div className="hero-stats"><strong>{storage.files.length}</strong><span>mock files</span><strong>{formatBytes(storage.totalBytes)}</strong><span>directory total</span></div>
      </section>

      <div className="storage-layout">
        <section className="panel file-tree" aria-label="Database files">
          <div className="section-heading"><div><p className="panel-kicker">Directory</p><h3>{state.path.split('/').at(-1)}</h3></div><Badge tone="warning">Mock</Badge></div>
        ...
        <section className="panel file-detail">
          {!selected ? <EmptyState detail="Open a mock session to generate storage files." title="No file selected" /> : (
            <>
