  ...
  return (
    <div className="page-stack">
      <section className="hero-panel">
        <div><p className="panel-kicker">Deterministic model experiment</p><h2>Workload runner</h2><p>Generate a repeatable operation stream. Goal 1 simulates execution; reference-map validation becomes authoritative with the C++ backend.</p></div>
      ...
      <div className="workload-layout">
        <section className="panel workload-config">
          ...
          {state.connection !== 'open' && <p className="form-hint">Open a mock database session before starting.</p>}
        </section>

        <section className="panel workload-progress">
          <div className="section-heading"><div><p className="panel-kicker">Run {workload.id}</p><h3>{workload.status === 'idle' ? 'No run started' : `${progress}% complete`}</h3></div><Badge tone="warning">Mock oracle</Badge></div>
          ...
          <div className={`oracle-result ${workload.mismatch ? 'oracle-failed' : ''}`}>
            ...
            <div><strong>{workload.mismatch ? 'First mismatch captured' : 'No model mismatch observed'}</strong><p>{workload.mismatch ? `${workload.mismatch.key}: expected ${workload.mismatch.expected}, received ${workload.mismatch.actual}` : 'Mock mode uses a deterministic model. Live validation is unavailable until Goal 3.'}</p></div>
          </div>
